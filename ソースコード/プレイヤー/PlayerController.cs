...
public class PlayerController : MonoBehaviour
{
    ...
    [Header("斬撃のオブジェクト")]public GameObject swordVFX;
    #endregion
    ...
    public void Attack()
    {
        ...
        // HPが満タンの時は、斬撃を飛ばせるようにする
        if (GameManager.instance.playerHp == 100)
            Instantiate(swordVFX, attackPoint.position, Quaternion.identity);
    }
    ...
    {
        if((Input.GetAxis("Fire1") > 0 || isPushAttackButton) && !isRun && !isJump)
        {
            isAttack = true;
        }
        else
    ...
    {
        // Q キー または ガードボタンを押していて、かつ攻撃している状態でないときにガードのモーションをする 
        if ((Input.GetKey(KeyCode.Q) || isPushGuardButton) && !isAttack)
        {
            isBlocking = true;
        }
        else
